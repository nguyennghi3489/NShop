...
App.Templates.gallery = [
	'<div class="nwrapper" style="width:<%= opts.width %>px;height:<%= opts.width*opts.ratio %>px;overflow:hidden;position:relative;">',
		'<ul class="nslider" style="width:<%=imageList.length * opts.width %>px<% %>;height:<%= opts.width*opts.ratio %>px;position:absolute;top:0px;">',
			'<% _.each(imageList,function(i){ %><li style="width:<%= opts.width %>px;height:100%;background-image:url(<%= i %>);background-repeat:none;background-size: 100%;float:left"></li> <% }) %>',
		'</ul>',
	'</div>',
	'<div class="control" style="width:<%= opts.width %>px;">',
		'<a class="prev" href="#"></a>',
		'<a class="next" href="#"></a>',
	'</div>',
	'<div class="thumbs">',
		'<ul class="nthumbslider square">',
